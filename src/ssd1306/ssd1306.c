...
// Desenho de pixels e caracteres
...
// Função para desenhar caracteres em tamanho grande
void ssd1306_draw_char_large(int x, int y, char c, bool color) {
    if (c < 32 || c > 126)
        return;
    ...
    for (int i = 0; i < 5; i++) {
        ...
        for (int j = 0; j < 8; j++) {
            if (line & 0x01) {
                // Desenha cada pixel ampliado (1.5x1.5)
                ssd1306_draw_pixel(x + i * 1.5, y + j * 1.5, color);
                ssd1306_draw_pixel(x + i * 1.5 + 1, y + j * 1.5, color);
                ssd1306_draw_pixel(x + i * 1.5, y + j * 1.5 + 1, color);
                ssd1306_draw_pixel(x + i * 1.5 + 1, y + j * 1.5 + 1, color);
            }
            line >>= 1;
        }
    }
...
// Função para desenhar caracteres em tamanho medias
void ssd1306_draw_char_mid(int x, int y, char c, bool color) {
    if (c < 32 || c > 126)
        return;
    ...
    for (int i = 0; i < 5; i++) {
        ...
        for (int j = 0; j < 8; j++) {
            if (line & 0x01) {
                // Desenha cada pixel ampliado (1.1x1.1)
                ssd1306_draw_pixel(x + i * 1.1, y + j * 1.1, color);
                ssd1306_draw_pixel(x + i * 1.1 + 1, y + j * 1.1, color);
                //ssd1306_draw_pixel(x + i * 1.1, y + j * 1.1 + 1, color);
                //ssd1306_draw_pixel(x + i * 1.1 + 1, y + j * 1.1 + 1, color);
            }
            line >>= 1;
        }
    }
...
// Função para desenhar strings grandes
void ssd1306_draw_string_large(int x, int y, const char *str, bool color) {
    while (*str) {
        ssd1306_draw_char_large(x, y, *str++, color);
        x += 9; // Avançar 12 pixels (5 de largura * 1.5 + espaço ampliado)
    }
}


// Função para desenhar strings medias
void ssd1306_draw_string_mid(int x, int y, const char *str, bool color) {
    while (*str) {
        ssd1306_draw_char_mid(x, y, *str++, color);
        x += 7; // Avançar 7 pixels (5 de largura * 1.1 + espaço ampliado)
    }
}
    ...
    for (int page = 0; page < h / 8; page++) {
        for (int col = 0; col < w; col++) {
            ...
            for (int bit = 0; bit < 8; bit++) {
                if (byte & (1 << bit)) {
                    ssd1306_draw_pixel(x + col, y + page * 8 + bit, color);
                }
            }
        }
    }
